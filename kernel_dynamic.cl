...
__kernel void _updatePlanet(__global Planet* inputPlanets, __global Planet* outputPlanets, __global const double* dt_G_planetCount) {
    ...
    for (int i = 0; i < planetCount; i++) {
        ...
        if(M_i > M){
            ...
            outputPlanets[i].mass = sumMM; //TODO add atomic
            outputPlanets[gid].mass = -1;
            outputPlanets[i].vx = 0; //TODO add atomic
            outputPlanets[i].vy = 0; //TODO add atomic
            outputPlanets[i].vz = 0; //TODO add atomic
            return;
        }
        continue;
        }
